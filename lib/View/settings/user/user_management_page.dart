...
class _UserManagementPageState extends State<UserManagementPage>
  ...
  Future<void> _loadInitial() async {
    ...
    try {
      ...
      _employees = employeesResponse.data ?? const <EmployeeModel>[];
  ...
  Future<void> _loadUser(int userId) async {
    ...
    _setEmployeeCode(user.employeeCode ?? '');
  ...
  EmployeeModel? get _selectedEmployee =>
      _employees.cast<EmployeeModel?>().firstWhere(
        (employee) => employee?.id == _selectedEmployeeId,
        orElse: () => null,
      );

  List<EmployeeModel> get _availableEmployees => _employees
      .where(
        (employee) =>
            employee.id == _selectedEmployeeId || employee.userId == null,
      )
      .toList(growable: false);
  ...
  void _selectEmployee(int? employeeId) {
    ...
    setState(() {
      ...
      _setEmployeeCode(employee?.employeeCode ?? '');

      if (_isNewUser && employee != null) {
        _emailController.text = _emailController.text.trim().isEmpty
            ? (employee.email ?? '')
            : _emailController.text;
        _mobileController.text = _mobileController.text.trim().isEmpty
            ? (employee.mobile ?? '')
            : _mobileController.text;
        _profilePhotoController.text =
            _profilePhotoController.text.trim().isEmpty
  ...
  Future<void> _saveProfile() async {
    ...
    if (_selectedEmployeeId == null) {
      setState(() {
        _formError = 'Please select an employee before saving the user.';
      });
      return;
    }
    ...
    return Column(
      children: [
        ...
        ListView(
          ...
          padding: const EdgeInsets.all(AppUiConstants.cardPadding),
          children: _effectivePermissions
              .map((permission) {
                final permissionKey =
                    '${permission.permissionId ?? permission.code ?? permission.name ?? 'permission'}';
                final index = _effectivePermissions.indexOf(permission);
  ...
  String _permissionRightsSummary(UserPermissionModel permission) {

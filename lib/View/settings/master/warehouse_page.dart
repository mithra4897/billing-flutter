...
class _WarehouseManagementPageState extends State<WarehouseManagementPage> {
  ...
  Future<void> _loadData({int? selectId}) async {
    ...
    try {
      ...
      setState(() {
        ...
        _filteredWarehouses = _filterWarehouses(warehouses);
  ...
  void _applySearch() {
    setState(() {
      _filteredWarehouses = _filterWarehouses(_warehouses);
    });
  }

  List<WarehouseModel> _filterWarehouses(List<WarehouseModel> items) {
    final scoped = items
        .where((warehouse) {
          if (widget.fixedCompanyId != null &&
              warehouse.companyId != widget.fixedCompanyId) {
            return false;
          }
          if (widget.fixedBranchId != null &&
              warehouse.branchId != widget.fixedBranchId) {
            return false;
          }
          return true;
        })
    ...
    return filterMasterList(scoped, _searchController.text, (warehouse) {
  ...
  void _selectWarehouse(WarehouseModel warehouse) {
    ...
    _setCode(warehouse.code ?? '', autoGenerated: false);
  ...
  bool get _isNewWarehouse => _selectedWarehouse?.id == null;

  String _warehouseTypeCode() {
    switch (_warehouseType) {
      case 'raw_material':
        return 'RM-STK';
      case 'finished_goods':
        return 'FG-STK';
  ...
  String _generateWarehouseCode() {
    if (_locationId == null) {
      return '';
    }

    final prefix = _warehouseTypeCode();
    final pattern = RegExp('^${RegExp.escape(prefix)}/(\\d+)\$');
    var nextNumber = 1;

    for (final warehouse in _warehouses.where(
      (item) => item.locationId == _locationId,
    )) {
      final match = pattern.firstMatch(
        (warehouse.code ?? '').trim().toUpperCase(),
      );
      if (match == null) {
        continue;
      }
  ...
  void _primeCodeSuggestion() {
    if (!_isNewWarehouse || _locationId == null) {
      return;
    }

    final code = _generateWarehouseCode();
          ...
          Wrap(
            ...
            children: [
              ...
              AppToggleChip(
                label: 'Sellable',
                value: _isSellableStock,
                onChanged: (value) => setState(() => _isSellableStock = value),
              ),
              AppToggleChip(
                label: 'Reserved Only',
                value: _isReservedOnly,

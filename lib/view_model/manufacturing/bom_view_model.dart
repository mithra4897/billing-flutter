...
class BomLineDraft {
  BomLineDraft({
    ...
    String? remarks,
       ...
       wastagePercentController =
           TextEditingController(text: wastagePercent ?? ''),
       lineTypeController =
           TextEditingController(text: lineType ?? 'raw_material'),
...
class BomOperationDraft {
  BomOperationDraft({
    ...
    String? runMinutes,
           ...
           TextEditingController(text: operationName ?? ''),
...
class BomViewModel extends ChangeNotifier {
  ...
  Future<void> load({int? selectId}) async {
    ...
    try {
      ...
      rows = (responses[0] as PaginatedResponse<BomModel>).data ??
          const <BomModel>[];
      companies =
      ...
      uoms = ((responses[5] as PaginatedResponse<UomModel>).data ??
          ...
          .where((x) => x.isActive)
          .toList(growable: false);
      uomConversions = ((responses[6] as PaginatedResponse<UomConversionModel>).data ??
  ...
  void resetDraft() {
    ...
    effectiveFromController.text = DateTime.now().toIso8601String().split('T').first;
  ...
  Future<void> select(BomModel row) async {
    ...
    try {
      ...
      bomCodeController.text = stringValue(data, 'bom_code');
  ...
  void addOperation() {
    if (!canEdit) return;
    operations = List<BomOperationDraft>.from(operations)..add(BomOperationDraft());
    notifyListeners();
  }

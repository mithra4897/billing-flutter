...
const List<AppDropdownItem<String>>
inventoryAdjustmentReasonItems = <AppDropdownItem<String>>[
  AppDropdownItem<String>(
...
class InventoryAdjustmentLineDraft {
  InventoryAdjustmentLineDraft({
    ...
    String? remarks,
       ...
       adjustmentQtyController = TextEditingController(
         text: adjustmentQty ?? '',
       ),
  ...
  Map<String, dynamic> toJson() {
    ...
    final direction =
        adjustmentDirection ?? (adjustmentQty >= 0 ? 'in' : 'out');
...
class InventoryAdjustmentViewModel extends ChangeNotifier {
    ...
    try {
      ...
      if (selectId != null) {
        final row = rows.cast<InventoryAdjustmentModel?>().firstWhere(
          (r) =>
              intValue(r?.toJson() ?? const <String, dynamic>{}, 'id') ==
              selectId,
          orElse: () => null,
        );
        if (row != null) {
  ...
  Future<void> selectRow(InventoryAdjustmentModel row) async {
    ...
    try {
      ...
      adjustmentDateController.text = displayDate(
  ...
  String? _validate() {
    ...
    if (documentSeriesId == null &&
        adjustmentNoController.text.trim().isEmpty) {
      return 'Either adjustment number or document series is required.';
    }
    if (adjustmentDateController.text.trim().isEmpty) {
      return 'Adjustment date is required.';
    }
    if (!inventoryAdjustmentTypeItems.any((e) => e.value == adjustmentType)) {
      return 'Invalid adjustment type.';
    }
    if (!inventoryAdjustmentReasonItems.any((e) => e.value == reasonCode)) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      final adjustmentQty = adjustmentQtyText.isEmpty
          ? actualQty - systemQty
          : (double.tryParse(adjustmentQtyText) ?? 0);
      final direction =
          line.adjustmentDirection ?? (adjustmentQty >= 0 ? 'in' : 'out');
      if (line.itemId == null ||
      ...
      if (adjustmentQty == 0) {
        return 'Adjustment quantity must not be zero at line $lineNo.';
      }
      if (direction != 'in' && direction != 'out') {
        return 'Invalid adjustment direction at line $lineNo.';
      }
      if (adjustmentType == 'increase' && direction != 'in') {
        return 'Increase adjustment can contain only inward lines (line $lineNo).';
      }
      if (adjustmentType == 'decrease' && direction != 'out') {
        return 'Decrease adjustment can contain only outward lines (line $lineNo).';
      }
      if (line.serialId != null && adjustmentQty.abs() != 1) {
        return 'Serial line quantity must be exactly 1 at line $lineNo.';
      }
    }
    return null;
  }

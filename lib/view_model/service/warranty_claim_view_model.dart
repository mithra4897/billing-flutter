...
class WarrantyClaimViewModel extends ChangeNotifier {
  ...
  final TextEditingController contractAssetIdController = TextEditingController();
  ...
  Map<String, dynamic> _buildCreatePayload() {
    ...
    return <String, dynamic>{
      ...
      if (assetId != null) 'service_contract_asset_id': assetId,
  ...
  Future<void> createWorkOrderFromClaim() async {
    ...
    try {
      ...
      if (response.success) {
        actionMessage =
            response.message.isNotEmpty ? response.message : 'Work order created.';
      } else {

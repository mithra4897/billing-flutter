...
class ProformaInvoiceLineDraft {
  ProformaInvoiceLineDraft({
    this.id,
    this.itemId,
    this.uomId,
    this.taxCodeId,
...
class SalesProformaInvoiceManagementController extends GetxController {
  SalesProformaInvoiceManagementController();

  static const String lineItemsSectionId = 'proforma_invoice_line_items';
  static final RegExp _revisionHeaderPattern = RegExp(
    r'^Revised Quote (\d+) for proformaInvoice (.+?)(?: dated .*)?$',
    multiLine: true,
  );
  ...
  List<SalesQuotationModel> quotations = const <SalesQuotationModel>[];
  ...
  SalesProformaInvoiceModel? selectedItem;
  ...
  bool get canEdit {
    ...
    return stringValue(selectedItem!.toJson(), 'proforma_status') == 'draft';
  }
  ...
  List<AppDropdownItem<int>> get quotationDropdownItems => quotationChoices
      ...
      .map(
        (item) => AppDropdownItem(
          value: item.id!,
          label:
              '${item.quotationNo ?? 'Quotation #${item.id}'} · ${displayDate(item.quotationDate)}',
        ),
      )
      .toList(growable: false);
    ...
    return documentSeries
        .where((item) {
          final typeOk =
              ...
              item.documentType == 'SALES_PROFORMA_INVOICE';
    ...
    try {
      ...
      itemPrices =
          ((responses[1] as PaginatedResponse<ItemPriceModel>).data ??
              ...
              .toList(growable: false);
      quotations =
          ((responses[2] as ApiResponse<List<SalesQuotationModel>>).data ??
              ...
              .where(
                (item) => const {
                  ...
                  'sent',
                  'accepted',
                }.contains((item.quotationStatus ?? '').toLowerCase()),
              )
              .toList(growable: false);
      itemLookupById
        ..clear()
  ...
  Future<void> selectDocument(
    SalesProformaInvoiceModel item, {
    bool notify = true,
  }) async {
    ...
    salesQuotationId = intValue(data, 'sales_quotation_id');
    customerPartyId = intValue(data, 'customer_party_id');
    billingAddressId = intValue(data, 'billing_address_id');
    ...
    proformaInvoiceNoController.text = stringValue(data, 'proforma_no');
  ...
  void resetForm({bool notify = true}) {
    ...
    termsController.text = documentTermsDefault('sales_proforma_invoice');
    isActive = true;
    _replaceLines(const <ProformaInvoiceLineDraft>[], notify: false);
    formError = null;
  ...
  void _applyFilters({bool notify = true}) {
    filteredItems =
        filterBySearchAndStatus(
              ...
              status: statusFilter,
              statusOf: (item) => stringValue(item.toJson(), 'proforma_status'),
              searchFieldsOf: (item) {
                final data = item.toJson();
                return <String>[
                  stringValue(data, 'proforma_no'),
            ...
            .where(
              (item) =>
                  matchesDateValueRange(
                    nullableStringValue(item.toJson(), 'proforma_date'),
  ...
  bool _matchesDashboardFilter(SalesProformaInvoiceModel item) {
    switch (dashboardFilter.trim()) {
      case 'open':
        ...
        return !<String>{'converted', 'cancelled'}.contains(status);
      default:
        return true;
    }
  }
  ...
  SalesLineTaxBreakdown taxBreakdownForLine(ProformaInvoiceLineDraft line) {
  ...
  Map<String, dynamic> linePayload(ProformaInvoiceLineDraft line) {
  ...
  DocumentPrintDataModel proformaInvoicePrintData() {
    ...
    return buildManagedDocumentPrintData(
      ...
      companyId: companyId,
      company: company,
      documentNumber: nullIfEmpty(proformaInvoiceNoController.text) ?? 'Draft',
      documentDate: proformaInvoiceDateController.text.trim(),
  ...
  void addLine() {
    lines = List<ProformaInvoiceLineDraft>.from(lines)
      ..add(ProformaInvoiceLineDraft());
    refreshComputedState();
  }

  void removeLine(int index) {
    final next = List<ProformaInvoiceLineDraft>.from(lines);
    next.removeAt(index);
    _replaceLines(next);
  }

  void _replaceLines(
    List<ProformaInvoiceLineDraft> nextLines, {
    bool notify = true,
  }) {
    replaceDisposableDraftEntries<ProformaInvoiceLineDraft>(
      previous: lines,
      next: nextLines,
      createEmpty: () => ProformaInvoiceLineDraft(),
  ...
  Future<void> save(BuildContext context) async {
    ...
    final payload = <String, dynamic>{
      ...
      'proforma_no': nullIfEmpty(proformaInvoiceNoController.text),
      'proforma_date': proformaInvoiceDateController.text.trim(),
    ...
    try {
      final response = selectedItem == null
          ? await _salesService.createProformaInvoice(
              SalesProformaInvoiceModel.fromJson(normalizeDatePayload(payload)),
            )
          : await _salesService.updateProformaInvoice(
      ...
      _refreshController.notifyChanged(source: 'sales_proforma_invoice');
    } catch (error) {
      formError = errorMessage(error);
      update();
    } finally {
  ...
  Future<void> docAction(
    BuildContext context,
    Future<ApiResponse<SalesProformaInvoiceModel>> Function() action,
  ) async {
    try {
  ...
  Future<void> deleteSelected(BuildContext context) async {
    ...
    try {
      final response = await _salesService.deleteProformaInvoice(id);
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(response.message)));
  ...
  Future<void> postSelected(BuildContext context) async {
    final id = intValue(selectedItem?.toJson() ?? const {}, 'id');
    if (id == null) return;
    await docAction(context, () => _salesService.postProformaInvoice(id));
  }
  ...
  Future<void> cancelSelected(BuildContext context) async {
    ...
    final reason = await promptCancellationReason(
      ...
      subjectLabel: selectedItem?.toString() ?? 'this sales proforma invoice',
    );
    if (reason == null || !context.mounted) return;
    await docAction(
      context,
      () => _salesService.cancelProformaInvoice(id, <String, dynamic>{
        'cancel_reason': reason,
      }),
    );
  }

  Future<void> convertSelected(BuildContext context) async {
    final id = selectedItem?.id;
    if (id == null || status != 'posted') return;
    formError = null;
    update();
    if (context.mounted) {
      openModuleShellRoute(context, '/sales/invoices/new?proforma_id=$id');
    }
  }
  ...
  String _revisionSourceNumberFromData(Map<String, dynamic> data) {
    ...
    return stringValue(data, 'proforma_no', 'Draft');
  }
  ...
  void reviseSelected(BuildContext context) {
    ...
    final nextLines = (data['lines'] as List<dynamic>? ?? const <dynamic>[])
        ...
        .map((line) {
          final copy = Map<String, dynamic>.from(line)
            ..remove('id')
            ..remove('sales_proforma_invoice_id')
            ..remove('created_at')
            ..remove('updated_at');
          return ProformaInvoiceLineDraft.fromJson(copy);
        })
        .toList(growable: true);

    final sourceNumber = _revisionSourceNumberFromData(data);
    final revisionNumber = _nextRevisionNumber(sourceNumber);
    final sourceDate = displayDate(nullableStringValue(data, 'proforma_date'));
    final revisionHeader = [
      'Revised Quote $revisionNumber for proformaInvoice $sourceNumber',
    ...
    documentSeriesId = intValue(data, 'document_series_id');
    salesQuotationId = intValue(data, 'id');
    customerPartyId = intValue(data, 'customer_party_id');
    isDirectCustomer = boolValue(data, 'is_direct_customer');
  ...
  void _disposeLines(List<ProformaInvoiceLineDraft> values) {

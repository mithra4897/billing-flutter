...
bool payslipCanSendEmailPdf(PayslipModel? payslip) {
  return payslip?.id != null &&
      payslip?.payrollRunStatus?.trim().toLowerCase() == 'posted';
}
...
Future<bool> sendPayslipEmailPdfFromRegister(
  ...
  if (!payslipCanSendEmailPdf(payslip)) {
    throw Exception('Payslip email is available after payroll is posted.');
  }
  if (!context.mounted) {
    return false;
  }

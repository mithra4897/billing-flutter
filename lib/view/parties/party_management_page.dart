...
class _PartyManagementPageState extends State<PartyManagementPage>
  ...
  Set<int> get _partyRoleTypeIds => _controller.partyRoleTypeIds;
          ...
          : (widget.startInNewMode
                ...
                : (_selectedParty == null
                      ...
                      : visibleParties.cast<PartyModel?>().firstWhere(
                          ...
                          orElse: () => visibleParties.isNotEmpty
                              ? visibleParties.first
                              : null,
                        )));
  ...
  String _partyTypeCode(int? id) {
    return partyCodePrefix(_partyTypeById(id));
  }
  ...
  String _generatePartyCodeForTypeFromParties(
    int? partyTypeId,
    Iterable<PartyModel> parties,
  ) {
    final typeCode = _partyTypeCode(partyTypeId);
    return generatePartyCode(
      typeCode: typeCode,
      series: _defaultPartySeries(),
      parties: parties,
    );
  }
  ...
  Future<String> _generatePartyCodeForType(int? partyTypeId) async {
    ...
    return _generatePartyCodeForTypeFromParties(partyTypeId, _parties);
  }

  void _setPartyCode(String value) {
  ...
  void _onPartyTypeChanged(int? value) {
    ...
    final savedCode = savedPartyCodeForOriginalType(_selectedParty, value);
    if (savedCode != null &&
        partyCodeUsesPrefix(savedCode, _partyTypeCode(value))) {
      _partyCodeRefreshToken++;
      _setPartyCode(savedCode);
      _controller.update();
      return;
    }

    unawaited(_refreshAutoGeneratedPartyCode(value));
  }

  void _onPartyTypesChanged(Set<int> values) {
    _controller.setPartyRoleTypeIds(values);
    if (values.isEmpty) {
      _controller.applyPartyTypeChange(
        null,
        supportsCompanyFlag: false,
        supportsGst: false,
      );
      return;
    }
    final primaryTypeId = values.contains(_partyTypeId)
        ? _partyTypeId
        : values.first;
    if (primaryTypeId != _partyTypeId) {
      _onPartyTypeChanged(primaryTypeId);
    }
  }

  Future<void> _refreshAutoGeneratedPartyCode(int? partyTypeId) async {
    final requestToken = ++_partyCodeRefreshToken;
    final generated = await _generatePartyCodeForType(partyTypeId);
    if (!mounted ||
        !isCurrentPartyCodeRefresh(
          requestToken: requestToken,
          latestRequestToken: _partyCodeRefreshToken,
          requestedPartyTypeId: partyTypeId,
          currentPartyTypeId: _partyTypeId,
        )) {
      return;
    }
    _setPartyCode(generated);
    _controller.update();
  }
  ...
  Future<void> _selectParty(PartyModel party) async {
    ...
    _setPartyCode(party.partyCode ?? '');
    final typeCode = _partyTypeCode(party.partyTypeId);
    if (!partyCodeUsesPrefix(party.partyCode, typeCode)) {
      unawaited(_refreshAutoGeneratedPartyCode(party.partyTypeId));
    }
  ...
  Future<void> _saveParty() async {
    if (_partyTypeId != null &&
        !partyCodeUsesPrefix(
          _partyCodeController.text,
          _partyTypeCode(_partyTypeId),
        )) {
      await _refreshAutoGeneratedPartyCode(_partyTypeId);
      if (!mounted) {
        return;
      }
    }
    ...
    final model = PartyModel(
      ...
      roles: _partyRoleTypeIds
          .map((typeId) => PartyRoleModel(partyTypeId: typeId, isActive: true))
          .toList(growable: false),
      ...
      openingBalance: Validators.parseFlexibleNumber(
  ...
  Future<void> _saveCreditLimit() async {
    ...
    try {
      final body = PartyCreditLimitModel.fromJson({
        ...
        'credit_limit': Validators.parseFlexibleNumber(
      ...
      child: Column(
        ...
        children: [
          ...
          SettingsFormWrap(
            children: [
              AppDropdownField<int>.fromMapped(
                labelText: 'Party Types',
                mappedItems: partyTypeItems,
                multiInitialValues: _partyRoleTypeIds,
                onMultiChanged: _onPartyTypesChanged,
                multiHintText: 'Search and select party types',
                isRequired: true,
                validator: (_) => _partyRoleTypeIds.isEmpty
                    ? 'Select at least one party type'
                    : null,
              ),
              AppFormTextField(

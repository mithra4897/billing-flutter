...
- Sales returns no longer increase invoice Paid Amount; credit-note allocations
  reduce only the outstanding balance.
- Sales invoices now retain Partially returned and Returned status even when
  their outstanding balance is zero.
- Due-date badge decoration no longer relabels returned invoices as Overdue.
- Added both return states to the Sales invoice filter and excluded them from
  Paid, Payment pending, Partially paid, and Overdue filter results.
- Added a dry-run-first backend reconciliation path for existing live records.
...
- Added centralized label rendering that removes legacy optional suffixes and
  automatically appends a red `*` for fields using built-in required
  validators, while still supporting explicit `isRequired` flags.
- Preserved existing callers through defaulted `isRequired` parameters and
  forwarded `ValidatedFormTextField` validation state to its base field.
- Added the design record in [dynamic-form-field-labels.md](dynamic-form-field-labels.md).

...
Give every Project register the same responsive, multi-filter experience used
by Sales while preserving the Project module's existing authorization and
company/project scoping.

### Scope and business rules

- Reuse `AppRegisterFilters`; do not create parallel filter-field styling or a
  second generic filter component.
- Standalone Project registers expose the shared filters from the top-bar
  Filter button. Embedded constrained subtabs keep the filter surface visible
  because they do not own a standalone top bar.
- Task and Milestone use the existing `AppRegisterFiltersSection` wrapper for
  the shared filter background, animation, and spacing.
- Keep search in each register's existing shell/list search control and combine
  it with the shared filter surface. When an embedded subtab has no shell
  actions, render search through the shared surface so filtering remains
  reachable.
- Projects filter by customer, status, project type, billing method, and
  expected-start date range.
- Tasks filter by project, assigned employee, status, priority, and planned-
  start date range.
- Milestones filter by project, status, and target-date range.
- Timesheets filter by project, task, employee, status, and work-date range.
- Expenses filter by project, task, supplier, status, category, and expense-
  date range.
- Resource Usage filters by project, task, asset, and usage-date range.
- Vendor Works filters by project, task, vendor, and status; no date filter is
  shown because vendor work has no persisted date field.
- Billings filter by project, status, billing basis, and billing-date range.
- Project-constrained subtabs must keep the selected project boundary fixed;
  their Project filter is hidden and Clear restores that constraint.
- Multi-select membership uses `Set` values. Filtering is local over the
  already-authorized, already-loaded rows and must not trigger duplicate API
  requests or widen visibility.

### Acceptance criteria

1. All eight Project registers render `AppRegisterFilters` with every relevant
   persisted filter dimension listed above.
2. Filters combine with search and with one another and update visible records
   immediately.
3. Clear removes every optional filter while preserving a constrained project.
4. Task and Milestone Kanban boards render correctly with multiple statuses
   selected.
5. Empty option collections do not render unusable dropdowns.
6. No API, database, permission, or routing contract changes.
7. Focused Flutter analysis and Project filter tests pass.

...
- `lib/controller/crm/crm_enquiries_controller.dart`
- `lib/controller/crm/crm_leads_controller.dart`
- `lib/controller/crm/crm_opportunities_controller.dart`
...
- 2026-07-13: Continued the medium-priority inventory wave by migrating `stock_batch_view_model.dart`, `stock_serial_view_model.dart`, `stock_movement_view_model.dart`, and `inventory_inquiry_management_controller.dart` to the shared cache for common companies, items, and warehouses. These screens still fetch stock batches, stock serials, stock movements, and inquiry results directly because those are transactional or user-triggered.
- 2026-07-13: Continued the medium-priority asset wave by migrating `asset_category_view_model.dart`, `asset_depreciation_run_view_model.dart`, `asset_disposal_management_controller.dart`, `fixed_asset_management_controller.dart`, and `asset_cost_center_management_controller.dart` to the shared cache for companies, document series, parties, and warehouses where applicable. These screens still fetch asset-specific transactions and option lists such as assets, categories, cost centers, departments, and employees directly where that data remains module-specific.
- 2026-07-13: Moved cache administration into a dedicated `Settings -> System -> Cache Controls` page instead of embedding it inside module preferences. The page now exposes cache enable/disable and clear-cache actions in one predictable admin location while `Module Preferences` returns to module-only behavior.
- 2026-07-13: Migrated `purchase_requisition_management_controller.dart`, `crm_enquiries_controller.dart`, `crm_leads_controller.dart`, and `crm_opportunities_controller.dart` to the shared cache for common companies, branches, locations, financial years, document series, parties, items, UOMs, UOM conversions, and warehouses where applicable. These screens still fetch purchase requisitions, CRM enquiries, CRM leads, CRM opportunities, users, departments, stages, and sources directly because those remain page-specific or workflow-specific.
- 2026-07-13: Migrated `email_rules_management_controller.dart`, `email_messages_management_controller.dart`, `email_module_settings_management_controller.dart`, `email_templates_management_controller.dart`, and `email_settings_management_controller.dart` to the shared cache for companies and document series. These screens still fetch email rules, templates, module settings, messages, and SMTP settings directly because those remain communication-module records rather than shared master data.
- 2026-07-13: Migrated `branch_management_controller.dart`, `business_location_management_controller.dart`, `warehouse_management_controller.dart`, and `document_series_management_controller.dart` to the shared cache for companies, branches, business locations, and financial years where applicable. These screens still fetch the editable branch, business-location, warehouse, and document-series registers directly because those are the module records being managed, not shared read-only master dependencies.
- 2026-07-13: Migrated `user_management_controller.dart`, `financial_year_management_controller.dart`, `item_supplier_map_management_controller.dart`, and `item_price_management_controller.dart` to the shared cache for companies, branches, locations, warehouses, items, party types, parties, UOMs, and UOM conversions where applicable. These screens still fetch users, employees, roles, permissions, financial-year registers, item-supplier mappings, and item-price rows directly because those remain editable module data rather than shared reference data.
- 2026-07-13: Migrated `account_management_controller.dart`, `budget_management_controller.dart`, and `document_posting_management_controller.dart` to the shared cache for companies, branches, locations, and financial years where applicable. These screens still fetch account groups, accounts, budgets, posting-rule groups, and document-posting rows directly because those remain accounting-module records rather than shared master datasets.
...
- 2026-07-18: Fixed newly created or updated master records remaining absent from cached dropdown data until a browser refresh. Base CRUD mutation responses are now upserted into all 14 loaded master datasets after the normal dataset refresh, making the server-returned record authoritative even if the follow-up list response is delayed or stale. Added focused regression tests covering every cached model type plus update and invalid-cache safety.
